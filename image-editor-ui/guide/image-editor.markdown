...
|forceCrop(AdobeImageIntent.ForceCrop)      |Forces the user to crop the image to a developer-specified ratio before making any other edits.<br><br>An example of creating a `forceCropSettings` argument:<br><br>`AdobeImageIntent.ForceCrop forceCropSettings = new AdobeImageIntent.ForceCrop.Builder()`<br>`    .addCropRatio("Square", "1:1")`<br>`    .addCropRatio("3:2", "3:2")`<br>`    .withSelectedIndex(0)`<br>`    .canInvert(true)`<br>`    .build();`<br>|
